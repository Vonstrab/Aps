use crate::ast;

use std::collections::HashMap;

#[derive(Clone, PartialEq,Debug)]
pub enum Value {
    Int(i64),
    Fermeture(Box<ast::AstExp>, Vec<String>, HashMap<String, Value>),
    FermetureRec(Box<ast::AstExp>, Vec<String>, HashMap<String, Value>),
}

impl Value {
    pub fn as_int(&self) -> i64 {
        use self::Value::*;
        match self {
            Int(i) => *i,
            _ => panic!("not a int"),
        }
    }

    pub fn to_bool(i: i64) -> bool {
        match i {
            1 => true,
            0 => false,
            _ => panic!("not a bool"),
        }
    }

    pub fn to_int(b: bool) -> i64 {
        match b {
            true => 1,
            false => 0,
        }
    }
}

impl ast::AstCdms {
    pub fn eval(&self, env: &mut HashMap<String, Value>) -> Vec<i64> {
        use ast::AstCdms::*;
        let mut flux_sortie: Vec<i64> = Vec::new();

        match self {
            Dec(dec, cdms) => {
                dec.eval(env);
                flux_sortie.append(&mut cdms.eval(env));
            }
            Stat(stat, cdms) => {
                flux_sortie.append(&mut stat.eval(env));
                flux_sortie.append(&mut cdms.eval(env));
            }

            FStat(stat) => {
                flux_sortie.append(&mut stat.eval(env));
            }
        }
        flux_sortie
    }
}

impl ast::AstDec {
    pub fn eval(&self, env: &mut HashMap<String, Value>) {
        use ast::AstDec::*;

        match self {
            ASTConst(ident, _, exp) => {
                env.insert(ident.clone(), exp.eval(&env));
            }
            ASTFunc(fname, _, a, e) => {
                let mut args = Vec::new();
                for arg in a {
                    args.push(arg.ident.clone());
                }
                let fenv = env.clone();
                env.insert(fname.clone(), Value::Fermeture(e.clone(), args, fenv));
                println!("On ajoute {} a l'env", fname);
            }
            ASTFuncRec(x, _, a, e) => {
                let mut args = Vec::new();
                for arg in a {
                    args.push(arg.ident.clone());
                }

                let mut fenv = env.clone();
                let ferm = Value::Fermeture(e.clone(), args.clone(), fenv.clone());
                fenv.insert(x.clone(), ferm.clone());
                env.insert(x.clone(), ferm.clone());
            }
            _ => {
                panic!("NOT YET");
            }
        }
    }
}

impl ast::AstStat {
    pub fn eval(&self, env: &HashMap<String, Value>) -> Vec<i64> {
        use ast::AstStat::*;
        let mut flux_sortie: Vec<i64> = Vec::new();

        match self {
            ASTEcho(expr) => flux_sortie.push(expr.eval(env).as_int()),
            _ => {
                panic!("NOT YET");
            }
        }
        flux_sortie
    }
}

impl ast::AstExp {
    pub fn eval(&self, env: &HashMap<String, Value>) -> Value {
        use ast::AstExp::*;

        match self {
            ASTInt(n) => Value::Int(*n),
            ASTBool(b) => Value::Int(Value::to_int(*b)),
            ASTUnPrim(_, e) => {
                let exp = e.eval(env);
                if exp == Value::Int(1) {
                    Value::Int(0)
                } else {
                    Value::Int(1)
                }
            }
            ASTBinPrim(op, e1, e2) => {
                use ast::Oprim;

                let expr1 = e1.eval(env).as_int();
                let expr2 = e2.eval(env).as_int();
                match op {
                    Oprim::Add => Value::Int(expr1 + expr2),
                    Oprim::Mul => Value::Int(expr1 * expr2),
                    Oprim::Div => Value::Int(expr1 / expr2),
                    Oprim::Sub => Value::Int(expr1 - expr2),
                    Oprim::Eq => Value::Int(Value::to_int(expr1 == expr2)),
                    Oprim::Lt => Value::Int(Value::to_int(expr1 < expr2)),
                    Oprim::And => Value::Int(Value::to_int(
                        Value::to_bool(expr1) && Value::to_bool(expr2),
                    )),
                    Oprim::Or => Value::Int(Value::to_int(
                        Value::to_bool(expr1) || Value::to_bool(expr2),
                    )),
                }
            }
            ASTIf(e1, e2, e3) => {
                if e1.eval(env).as_int() == 1 {
                    e2.eval(env)
                } else {
                    e3.eval(env)
                }
            }
            ASTIdent(x) => env[x].clone(),
            ASTApp(e, args) => {
                println!("env avant l'appel de {:?} : {:?}", env, e);
                match &env[e] {
                    Value::Fermeture(fbody, fargs, fenv) => {
                        let mut nfenv = fenv.clone();

                        for i in 0..fargs.len() {
                            nfenv.insert(fargs[i].clone(), args[i].eval(env));
                        }
                        fbody.eval(fenv)
                    }
                    Value::FermetureRec(fbody, fargs, fenv) => {
                        let mut nfenv = fenv.clone();
                        for i in 0..fargs.len() {
                            nfenv.insert(fargs[i].clone(), args[i].eval(env));
                        }
                        fbody.eval(fenv)
                    }
                    _ => panic!("not a fermeture"),
                }
            }
            _ => {
                panic!("NOT YET");
            }
        }
    }
}

//     ASTEntier(n) => *n,
//     ASTFalse() => 0,
//     ASTTrue() => 1,
//     ASTUnPrim(_, e) => {
//         let exp = e.eval(env);
//         if exp == 1 {
//             0
//         } else {
//             1
//         }
//     }
//     ASTEcho(e) => {
//         let exp = e.eval(env);
//         println!("{}", exp);
//         0
//     }
//     ASTBinPrim(op, e1, e2) => {
//         let expr1 = e1.eval(env);
//         let expr2 = e2.eval(env);
//         match op {
//             Oprim::Add => expr1 + expr2,
//             Oprim::Mul => expr1 * expr2,
//             Oprim::Div => expr1 / expr2,
//             Oprim::Sub => expr1 - expr2,
//             Oprim::Eq => to_int(expr1 == expr2),
//             Oprim::Lt => to_int(expr1 < expr2),
//             Oprim::And => to_int(to_bool(expr1) && to_bool(expr2)),
//             Oprim::Or => to_int(to_bool(expr1) || to_bool(expr2)),
//         }
//     }
//     ASTConst(x, _ty, e) => {
//         let e = e.eval(env);
//         env.insert(x.clone(), Value::Int(e));
//         0
//     }
//     ASTCdms(d, cds) => {
//         let _ = d.eval(env);
//         cds.eval(env)
//     }
//     ASTIf(e1, e2, e3) => {
//         if e1.eval(env) == 1 {
//             e2.eval(env)
//         } else {
//             e3.eval(env)
//         }
//     }
//     ASTPair(e1, e2) => match e1.as_ref() {
//         Ast::ASTIdent(x) => match &env[&x.clone()] {
//             Value::Fermeture(e, args, fenv) => {
//                 let mut nfenv = fenv.clone();
//                 nfenv.insert(args[0].clone(), Value::Int(e2.eval(& mut env.clone())));
//                 e.eval(&mut nfenv)
//             }
//             Value::Int(e) => *e,
//         },
//         _ => {
//             e1.eval(env);
//             e2.eval(env)
//         }
//     },
//     ASTIdent(x) => match &env[x] {
//         Value::Fermeture(_e, _args, _fenv) => 0,
//         Value::Int(e) => *e,
//     },
//     ASTFunc(x, t, a, e) => {
//         let mut args = Vec::new();
//         for arg in a {
//             match arg {
//                 Arg::Arg((x, t)) => args.push(x.clone()),
//             }
//         }

//         let fenv = env.clone();
//         env.insert(x.clone(), Value::Fermeture(e.clone(), args, fenv));
//         0
//     }
//     ASTFuncRec(x, t, a, e) => {
//         let mut args = Vec::new();
//         for arg in a {
//             match arg {
//                 Arg::Arg((x, t)) => args.push(x.clone()),
//             }
//         }

//         let mut fenv = env.clone();
//         let ferm =Value::Fermeture(e.clone(), args.clone(), fenv.clone());
//         fenv.insert(x.clone(), ferm.clone());
//         env.insert(x.clone(),ferm.clone()) ;
//         0
//     }

//     _ => {
//         panic!("NOT YET");
//     }
// }
